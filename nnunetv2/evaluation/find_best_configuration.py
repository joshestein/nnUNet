...
def filter_available_models(model_dict: Union[List[dict], Tuple[dict, ...]], dataset_name_or_id: Union[str, int]):
    ...
    for trained_model in model_dict:
        plans_manager = PlansManager(
            join(
                nnUNet_preprocessed, maybe_convert_to_dataset_name(dataset_name_or_id), trained_model["plans"] + ".json"
            )
        )
...
def find_best_configuration(
    ...
    # print it
    print_inference_instructions(
        return_dict, instructions_file=join(nnUNet_results, dataset_name, "inference_instructions.txt")
    )
    return return_dict
...
def find_best_configuration_entry_point():
    ...
    parser.add_argument(
        "-p", nargs="+", required=False, default=["nnUNetPlans"], help="List of plan identifiers. Default: nnUNetPlans"
    )
    parser.add_argument(
        "-c",
        nargs="+",
    ...
    parser.add_argument(
        "-tr", nargs="+", required=False, default=["nnUNetTrainer"], help="List of trainers. Default: nnUNetTrainer"
    )
    parser.add_argument(
        "-np",
        required=False,
    ...
    parser.add_argument("-f", nargs="+", type=int, default=(0, 1, 2, 3, 4), help="Folds to use. Default: 0 1 2 3 4")
    parser.add_argument(
        "--disable_ensembling", action="store_true", required=False, help="Set this flag to disable ensembling"
    )
    parser.add_argument(
        "--no_overwrite",
        action="store_true",
...
def accumulate_crossval_results_entry_point():
    parser = argparse.ArgumentParser(
        "Copies all predicted segmentations from the individual folds into one joint " "folder and evaluates them"
    )
    parser.add_argument("dataset_name_or_id", type=str, help="Dataset Name or id")
    ...
    parser.add_argument(
        "-tr", type=str, required=False, default="nnUNetTrainer", help="Trainer class. Default: nnUNetTrainer"
    )
    args = parser.parse_args()
