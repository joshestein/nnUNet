...
def determine_postprocessing(
    ...
    with multiprocessing.get_context("spawn").Pool(num_processes) as pool:
        ...
        if len(labels_or_regions) > 1:
            for label_or_region in labels_or_regions:
                ...
                pp_results = load_summary_json(join(output_here, "summary.json"))
                do_this = (
                    pp_results["mean"][label_or_region]["Dice"] > baseline_results["mean"][label_or_region]["Dice"]
                )
                if do_this:
                    print(
    ...
    tmp["input_folder"]["mean"] = {
        label_or_region_to_key(k): tmp["input_folder"]["mean"][k] for k in tmp["input_folder"]["mean"].keys()
    }
    tmp["postprocessed"]["mean"] = {
        label_or_region_to_key(k): tmp["postprocessed"]["mean"][k] for k in tmp["postprocessed"]["mean"].keys()
    }
...
if __name__ == "__main__":
    trained_model_folder = (
        "/home/fabian/results/nnUNet_remake/Dataset004_Hippocampus/nnUNetTrainer__nnUNetPlans__3d_fullres"
    )
    ...
    fns, kwargs = determine_postprocessing(
        merged_output_folder, labelstr, plans_manager.plans, dataset_json, 8, keep_postprocessed_files=True
    )
    ...
    apply_postprocessing_to_folder(
        merged_output_folder, merged_output_folder + "_pp", fns, kwargs, plans_manager.plans, dataset_json, 8
    )
    compute_metrics_on_folder(
        labelstr,
