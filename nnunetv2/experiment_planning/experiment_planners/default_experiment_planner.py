...
class ExperimentPlanner(object):
    ...
    def plan_experiment(self):
        ...
        if new_median_shape_transposed[0] != 1:
            ...
            while num_voxels_in_patch / median_num_voxels < self.lowres_creation_threshold:
                ...
                median_num_voxels = np.prod(
                    plan_3d_fullres["spacing"] / lowres_spacing * new_median_shape_transposed, dtype=np.float64
                )
        ...
        # per file.
        shutil.copy(
            join(self.raw_dataset_folder, "dataset.json"), join(nnUNet_preprocessed, self.dataset_name, "dataset.json")
        )
