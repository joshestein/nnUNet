...
class nnUNetTrainerDA5(nnUNetTrainer):
    ...
    def get_training_transforms(
        ...
        if np.any(matching_axes > 1):
            tr_transforms.append(
                Rot90Transform((0, 1, 2, 3), axes=valid_axes, data_key="data", label_key="seg", p_per_sample=0.5),
            )
        ...
        tr_transforms.append(
            OneOfTransform(
                [
                    MedianFilterTransform((2, 8), same_for_each_channel=False, p_per_sample=0.2, p_per_channel=0.5),
                    GaussianBlurTransform(
                        (0.3, 1.5), different_sigma_per_channel=True, p_per_sample=0.2, p_per_channel=0.5
                    ),
                ]
            )
        )
        ...
        tr_transforms.append(
            SharpeningTransform(strength=(0.1, 1), same_for_each_channel=False, p_per_sample=0.2, p_per_channel=0.5)
        )
        ...
        if regions is not None:
            ...
            tr_transforms.append(
                ConvertSegmentationToRegionsTransform(
                    list(regions) + [ignore_label] if ignore_label is not None else regions, "target", "target"
                )
            )
    ...
    def get_dataloaders(self):
        ...
        if allowed_num_processes == 0:
            ...
            mt_gen_train = LimitedLenWrapper(
                self.num_iterations_per_epoch, dl_tr, tr_transforms, allowed_num_processes, 6, None, True, 0.02
            )
