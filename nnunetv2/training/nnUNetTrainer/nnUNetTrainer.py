        ...
        if self.local_rank == 0:
            try:
                ...
                g = hl.build_graph(
                    self.network,
                    torch.rand(
                        (1, self.num_input_channels, *self.configuration_manager.patch_size), device=self.device
                    ),
                    transforms=None,
                )
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            if self.fold < len(splits):
                ...
                self.print_to_log_file(
                    "This random 80:20 split has %d training and %d validation cases." % (len(tr_keys), len(val_keys))
                )
                ...
                if next_stages is not None:
                    for n in next_stages:
                        ...
                        try:
                            ...
                            tmp = nnUNetDataset(
                                expected_preprocessed_folder, [k], num_images_properties_loading_threshold=0
                            )
